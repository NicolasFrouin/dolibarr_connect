...
/**
 *   	\file       verificationtoken_list.php
 *		\ingroup    connect
 *		\brief      List page for verificationtoken
 */
...
require_once __DIR__.'/class/verificationtoken.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('connect', 'verificationtoken', 'write');
	$permissiontodelete = $user->hasRight('connect', 'verificationtoken', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("connect")) {
	accessforbidden('Module connect not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'VerificationToken';
	$objectlabel = 'VerificationToken';
	$uploaddir = $conf->connect->dir_output;
...
$title = $langs->trans("VerificationTokens");
//$help_url = "EN:Module_VerificationToken|FR:Module_VerificationToken_FR|ES:Módulo_VerificationToken";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/connect/verificationtoken_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-connect page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/connect/verificationtoken_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendVerificationTokenRef";
$modelmail = "verificationtoken";
$objecttmp = new VerificationToken($db);
