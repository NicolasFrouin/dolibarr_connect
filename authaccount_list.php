...
/**
 *   	\file       authaccount_list.php
 *		\ingroup    connect
 *		\brief      List page for authaccount
 */
...
require_once __DIR__.'/class/authaccount.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('connect', 'authaccount', 'write');
	$permissiontodelete = $user->hasRight('connect', 'authaccount', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'AuthAccount';
	$objectlabel = 'AuthAccount';
...
$title = $langs->trans("AuthAccounts");
//$help_url = "EN:Module_AuthAccount|FR:Module_AuthAccount_FR|ES:Módulo_AuthAccount";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/connect/authaccount_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/connect/authaccount_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendAuthAccountRef";
$modelmail = "authaccount";
$objecttmp = new AuthAccount($db);
